export function buildEarningsPrepRequest(params: {
  ticker: string;
  quarter?: string;
  question: string;
}) {
  const { ticker, quarter, question } = params;
  const quarterText = quarter ? `for ${quarter}` : '';

  return {
    agent_id: 'earnings-prep-agent-v1', // configure this in Anthropic console
    input: {
      type: 'message',
      role: 'user',
      content: [
